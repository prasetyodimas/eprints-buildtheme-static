...
Control.Slider = Class.create({
  initialize: function (handle, track, options) {
    ...
    this.spans = this.options.spans
      ? this.options.spans.map(function (s) {
          return $(s);
        })
      : false;
    ...
    this.allowedValues = this.options.values
      ? this.options.values.sortBy(Prototype.K)
      : false;
      ...
      slider.setValue(
        parseFloat(
          (Object.isArray(slider.options.sliderValue)
            ...
            : slider.options.sliderValue) || slider.range.start
        ),
        i
      );
  ...
  maximumOffset: function () {
    return this.isVertical()
      ? (this.track.offsetHeight != 0
          ? this.track.offsetHeight
          : this.track.style.height.replace(/px$/, "")) - this.alignY
      : (this.track.offsetWidth != 0
  ...
  setSpan: function (span, range) {
    if (this.isVertical()) {
      ...
      span.style.height = this.translateToPx(
        range.end - range.start + this.range.start
      );
    } else {
      span.style.left = this.translateToPx(range.start);
      span.style.width = this.translateToPx(
        range.end - range.start + this.range.start
      );
    }
  },
  ...
  draw: function (event) {
    ...
    this.event = event;
    this.setValue(
      this.translateToValue(this.isVertical() ? pointer[1] : pointer[0])
    );
...
Event.observe(window, "load", function () {
  ...
  Event.observe(body, "dragenter", function (evt) {
    if (depth == 0) Event.fire(body, "ep:dragcommence", { event: evt });
    depth++;
    Event.stop(evt);
  });
  Event.observe(body, "dragleave", function (evt) {
    depth--;
    if (depth == 0) Event.fire(body, "ep:dragfinish", { event: evt });
  });
...
Object.extend(Prototype.Browser, {
  IE6: /MSIE (\d+\.\d+);/.test(navigator.userAgent)
    ? Number(RegExp.$1) == 6
      ...
      : false
    : false,
  IE7: /MSIE (\d+\.\d+);/.test(navigator.userAgent)
    ? Number(RegExp.$1) == 7
      ? true
      : false
    : false,
  IE8: /MSIE (\d+\.\d+);/.test(navigator.userAgent)
    ? Number(RegExp.$1) == 8
      ? true
      : false
    : false,
  IE9: /MSIE (\d+\.\d+);/.test(navigator.userAgent)
    ? Number(RegExp.$1) == 9
      ? true
      : false
    : false,
...
LightboxOptions = Object.extend(
  {
    ...
    fileBottomNavCloseImage:
      eprints_http_root + "/style/images/lightbox/closelabel.gif",
...
Lightbox.prototype = {
  ...
  updateMovie: function (src) {
    ...
    if (is_video) source.setAttribute("type", "video/mp4");
    else source.setAttribute("type", "audio/mp4");
...
function human_filesize(size_in_bytes) {
  if (size_in_bytes < 4096) return size_in_bytes + "b";

  var size_in_k = Math.floor(size_in_bytes / 1024);

  if (size_in_k < 4096) return size_in_k + "Kb";

  var size_in_meg = Math.floor(size_in_k / 1024);

  if (size_in_meg < 4096) return size_in_meg + "Mb";

  var size_in_gig = Math.floor(size_in_meg / 1024);

  if (size_in_gig < 4096) return size_in_gig + "Gb";
...
function EPJS_ShowPreview(e, preview_id) {
  ...
  Element.clonePosition(box, elt, {
    setWidth: false,
    offsetLeft: -Element.getWidth(box),
  }); //on the left
}
...
window.document.write(
  "<style>body { height: " + (window.innerHeight + 1) + "px; }</style>"
);
  ...
  if (current_vis) {
    ...
    new Effect.Scale(element_id, 0, {
      ...
      scaleMode: {
        ...
        originalWidth: inner.offsetWidth,
      },
    });
  } else {
    inner = $(element_id + "_inner");
    ...
    new Effect.Scale(element_id, 100, {
      scaleX: false,
      scaleContent: false,
      scaleFrom: 0,
      duration: 0.3,
...
var EPJS_Menus = Class.create({
    ...
    $$("a[menu]").each(
      function (el) {
        ...
        Event.observe(
          el,
          "mouseover",
          this.open.bindAsEventListener(this, menu_id)
        );
        Event.observe(
          el,
          "mouseout",
          this.close_timeout.bindAsEventListener(this, menu_id)
        );

        Event.observe(
          menu,
          "mouseover",
          this.cancel_timeout.bindAsEventListener(this, menu_id)
        );
        Event.observe(
          menu,
          "mouseout",
  ...
  open: function (event) {
    ...
    if (this.current_menu != null) {
      ...
      $(this.anchors.get(this.current_menu.id)).removeClassName(
        "ep_tm_menu_selected"
      );
    }
  ...
  close_timeout: function (event) {
    this.timer_id = Element.hide.delay(
      this.default_timeout,
      this.current_menu.id
    );
    this.anchor_timer_id = Element.removeClassName.delay(
      this.default_timeout,
      this.anchors.get(this.current_menu.id),
      "ep_tm_menu_selected"
    );
  },
...
var Metafield = Class.create({
  ...
  initialize: function (prefix, name, opts) {
    ...
    if (this.spaces) this.spaces = this.spaces.value;
  ...
  initialize_input_bits: function (basename) {
    ...
    this.root.select('input[type="text"]', "textarea").each(
      function (input) {
        if (input.id.match(match)) {
          var sub_name = input.id.substring(basename.length);
          if (sub_name.length) this.input_bits.push(sub_name);
        }
      }.bind(this)
    );
  },
          ...
          new Effect.SlideUp(dd, {
            ...
            afterFinish: function () {
              this.descendants().each(function (ele) {
                if (ele.nodeName == "DT") ele.removeClassName(className);
                if (ele.nodeName == "DD") ele.hide();
              });
            }.bind(dd),
          });
        } else {
...
var EPrintsProgressBar = Class.create({
  initialize: function (opts, container) {
    ...
    this.opts.border =
      opts.border == null ? "progress_border.png" : opts.border;
  ...
  onload: function () {
    this.img.setStyle({
      background:
        "url(" + this.img_path + this.opts.bar + ") top left no-repeat",
    });
  ...
  update: function (progress, alt) {
    ...
    this.timer = new PeriodicalExecuter(this._update.bind(this), 0.1);
    this._update();
  },
  _update: function () {
    ...
    if (x_offset == this.current) this.timer.stop();
    else this.current = x_offset;
    this.img.setStyle({
...
var Component_Field = Class.create({
  ...
  initialize_internal: function () {
    this.root.select("input.epjs_ajax").each(
      function (input) {
        if (input.type == "image") {
...
var Screen_EPrint_UploadMethod_File = Class.create({
  ...
  dragCommence: function (evt) {
    ...
    if (
      ...
      event.dataTransfer.types[0] == "application/x-moz-file"
    ) {
      ...
      eprints.currentRepository().phrase(
        ...
        function (phrases) {
          if (
            ...
            for (var i = 0; i < files.length; ++i) this.createFile(files[i]);
        }.bind(this)
      );
    } else for (var i = 0; i < files.length; ++i) this.createFile(files[i]);
  },
  /*
  ...
  postFile: function (file, offset) {
    ...
    new Ajax.Request(url, {
      ...
      requestHeaders: {
        "Content-Range":
          "" + offset + "-" + (offset + buffer.size) + "/" + file.size,
...
function UploadMethod_file_change(input, component, prefix) {
  ...
  if (input.value) {
    ...
    eprints
      .currentRepository()
      .phrase({ "lib/submissionform:action_cancel": {} }, function (phrases) {
...
function UploadMethod_update_progress_bar(uuid, container, pe) {
  var url =
    eprints_http_cgiroot + "/users/ajax/upload_progress?progressid=" + uuid;
  new Ajax.Request(url, {
    method: "get",
    onException: function (req, e) {
    ...
    onSuccess: function (transport) {
      ...
      if (pe && json.received == json.size) pe.stop();
    },
    onFailure: function (transport) {
      alert(
        "Request for " +
...
var Component_Documents = Class.create(Lightbox, {
  ...
  initialize_panel: function (panel) {
    ...
    panel.select('input[rel="interactive"]', 'input[rel="automatic"]').each(
      function (input) {
        ...
        Event.observe(
          link,
          "click",
          this.start.bindAsEventListener(this, link, docid, type)
        );
  ...
  start: function (event, input, docid, type) {
    ...
    $("prevLink").hide();
    $("nextLink").hide();
    $("imageDataContainer").setStyle({ opacity: 0.0001 });
    this.lightbox
      .setStyle({ top: lightboxTop + "px", left: lightboxLeft + "px" })
      .show();
    ...
    new Ajax.Request(url, {
      ...
      onSuccess: function (transport) {
        ...
        if (boxWidth == null || boxWidth < 640) boxWidth = 640;
        ...
        if (!form.onsubmit) {
          ...
          form
            .select('input[type="submit"]', 'input[type="image"]')
            .each(function (input) {
              input.observe(
                "click",
  ...
  update_messages: function (json) {
    ...
    container.update("");
    for (var i = 0; i < json.length; ++i) container.insert(json[i]);
  },
  remove_document: function (docid) {
    var doc_div = this.find_document_div(docid);
    if (!doc_div) return false;
    ...
    for (var i = 0; i < corder.length; ++i)
      for (var j = 0; j <= json.length; ++j)
        if (j == json.length) {
          ...
          corder.splice(i, 1);
          --i;
        } else if (corder[i] == json[j].id) break;
    ...
    for (var i = 0; i < json.length; ++i)
      for (var j = 0; j <= corder.length; ++j)
        if (json[i].refresh) {
          ...
          break;
        } else if (json[i].id == corder[j]) break;
    ...
    new Ajax.Request(url, {
      ...
      onFailure: function (transport) {
        if (transport.status == 404)
          if (this.remove_document(docid)) this.initialize_sortable();
      }.bind(this),
      parameters: params,
    });
  },
...
var EPJS_Stats = Class.create({
  ...
  run_next: function () {
    ...
    if (--EPJS_Stats_running_size < 0) EPJS_Stats_running_size = 0;
  ...
  draw: function () {
    ...
    if (this.can_run()) this.run(this);
    else this.queue(this);
  },
    ...
    if (response != null && response.status == 401) {
      if (this.container_id != null) {
        $(this.container_id).update(
          '<p class="irstats2_error_login">You must <a href="/cgi/users/login?target=' +
            document.URL +
            '">login</a> to access Statistics</p>'
        );
  ...
  wait: function () {
    ...
    $(this.container_id).insert(
      '<span class="irstats2_loading">Loading...</span>'
    );
  },
});
...
var EPJS_Set_Finder = function (
  ...
  if (context.from != null) target_url += "&from=" + context.from;
  if (context.to != null) target_url += "&to=" + context.to;
  ...
  if (!$(container_id).visible()) {
    $(container_id).update(
      new Element("img", { border: "0", src: "/style/images/loading.gif" })
    );
    $(container_id).show();
  }
...
var EPJS_Set_Finder_Autocomplete = function (
  container_id,
  select_id,
  input_id,
  base_url,
  ...
  if (input != null && content != null) {
    if (!content.visible()) {
      if (EPJS_Stats_Action_Toggled_Element.input_el != null) {
        ...
        EPJS_Stats_Action_Toggle(
          selected_element.input_el,
          selected_element.content_el,
          selected_element.extra_css
        );
      }

      Effect.SlideDown(content, { duration: 0.5 });
      if (extra_css != null) input.addClassName(extra_css);
      EPJS_Stats_Action_Toggled_Element = {
        input_el: input_el,
      ...
      content.hide();
      if (extra_css != null) input.removeClassName(extra_css);
      EPJS_Stats_Action_Toggled_Element = {};
    }
  }
  return false;
...
(function () {
  ...
  function datePicker(options) {
    ...
    o.events = {
      ...
      onclick: function (e) {
        if (this.className.search(/out-of-range|day-disabled/) != -1)
          return false;
        if (e == null) e = document.parentWindow.event;
        var el = e.target != null ? e.target : e.srcElement;
...
/* From: /usr/share/eprints3/archives/repositori2/cfg/static/javascript/auto/90_local.js */
    ...
    new Ajax.Request(url, {
      ...
      onFailure: function (transport) {
        throw new Error(
          "Error " +
            transport.status +
            " requesting phrases (check server log for details)"
        );
      },
      onSuccess: function (transport) {
